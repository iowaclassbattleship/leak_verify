...
The dataset counterpart is `../provenance` in this repository.

Run from the repository root:

```sh
go run ./custodial                          # http://127.0.0.1:8080
go run ./custodial -addr 127.0.0.1:9000     # other port
go test ./custodial/...                     # robustness tests
```

The marking key is random per process, so restarting the server clears all state.

The frontend is served straight from `custodial/web/`, not bundled into the binary: edit an HTML, CSS or JS file and just refresh the browser. Go changes still need a restart. Run from the project directory, or point `-web` at the directory:

```sh
go run ./custodial -web /path/to/custodial/web
```
...
Security Office sees the mark IDs that tie a recovered copy to a person;
everyone else sees them as `restricted`.
